...
export const freezeProgramTest = async function (freeze: boolean, authority: Signer, controller: Controller) {
  ...
  try {
    ...
    expect(isProgramFrozen_post).equals(freeze, "program freeze state is updated");
    console.log(`🧾 Previous freeze state of program is`, isProgramFrozen, "now is", isProgramFrozen_post);
    console.groupEnd();
  } catch (error) {
    console.error("❌", error);
    console.groupEnd();
