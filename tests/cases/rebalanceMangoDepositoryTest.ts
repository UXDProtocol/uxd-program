...
import { collateralUIPriceInMangoQuote, mintWithMangoDepository, rebalanceMangoDepository } from "../api";
import { CLUSTER, MANGO_QUOTE_DECIMALS, slippageBase, uxdHelpers } from "../constants";
import { getSolBalance, getBalance } from "../utils";

export const rebalanceMangoDepositoryTest = async (maxRebalancingAmount: number, slippage: number, user: Signer, controller: Controller, depository: MangoDepository, mango: Mango, payer?: Signer): Promise<number> => {
    console.group("🧭 rebalanceMangoDepositoryTest");
    try {

        const txId = await rebalanceMangoDepository(user, payer ?? user, maxRebalancingAmount, slippage, controller, depository, mango);
