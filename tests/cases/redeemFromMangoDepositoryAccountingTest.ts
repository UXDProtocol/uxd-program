...
export const redeemFromMangoDepositoryAccountingTest = async function (
  redeemableAmount: number,
  slippage: number,
  collateralUnitShift: number,
  user: Signer,
  ...
  try {
    ...
    const controllerRedeemable = nativeToUi(
    ...
    const txId = await redeemFromMangoDepository(
      user,
      payer ?? user,
      slippage,
      redeemableAmount,
    ...
    console.log(
      "🪙  perp price is",
      Number(mangoPerpPrice.toFixed(depository.quoteMintDecimals)),
      depository.quoteMintSymbol
    );
    ...
    const estimatedAmountRedeemableLostInSlippage =
      Math.abs(redeemableDelta - redeemableProcessedByRedeeming) - estimatedAmountRedeemableLostInTakerFees;
    ...
    const depositoryRedeemable_post = nativeToUi(
      depositoryAccount_post.redeemableAmountUnderManagement.toNumber(),
      depository.quoteMintDecimals
    );
    const depositoryCollateral_post = nativeToUi(
      depositoryAccount_post.collateralAmountDeposited.toNumber(),
      depository.collateralMintDecimals
    );
    const controllerAccount_post = await controller.getOnchainAccount(getConnection(), TXN_OPTS);
    const controllerRedeemable_post = nativeToUi(
      controllerAccount_post.redeemableCirculatingSupply.toNumber(),
      controller.redeemableMintDecimals
    );
    const depositoryTakerFees_post = nativeToUi(
      depositoryAccount_post.totalAmountPaidTakerFee.toNumber(),
      depository.quoteMintDecimals
    );
    ...
    console.log(
      ...
      "by burning",
      Number(redeemableProcessedByRedeeming.toFixed(controller.redeemableMintDecimals)),
      controller.redeemableMintSymbol,
      "(+~ takerFees =",
      Number(estimatedAmountRedeemableLostInTakerFees.toFixed(controller.redeemableMintDecimals)),
      controller.redeemableMintSymbol,
      ", +~ slippage =",
      Number(estimatedAmountRedeemableLostInSlippage.toFixed(controller.redeemableMintDecimals)),
      controller.redeemableMintSymbol,
      ")",
      "(frictionless redeeming would have been",
      Number(estimatedFrictionlessCollateralDelta.toFixed(controller.redeemableMintDecimals)),
      depository.collateralMintSymbol,
      ")",
      "|| odd lot returns ",
      Number(redeemableLeftOverDueToOddLot.toFixed(depository.collateralMintDecimals)),
      controller.redeemableMintSymbol,
      ")"
    );
    // Accounting
    expect(depositoryRedeemable_post).closeTo(
      depositoryRedeemable - redeemableDelta,
      redeemableNativeUnitPrecision,
      "Depository RedeemableAmountUnderManagement is incorrect"
    );
    expect(controllerRedeemable_post).closeTo(
      controllerRedeemable - redeemableDelta,
      redeemableNativeUnitPrecision,
      "Controller RedeemableCirculatingSupply is incorrect"
    );
    expect(depositoryTakerFees_post).to.be.within(
      depositoryTakerFees,
      depositoryTakerFees + mangoTakerFee * collateralDelta * mangoPerpPrice,
      "Depository TotalAmountPaidTakerFee is incorrect"
    );
    expect(depositoryCollateral_post).closeTo(
      depositoryCollateral - collateralDelta,
      collateralNativeUnitPrecision,
      "Depository CollateralAmountDeposited is incorrect"
    );
