...
export const redeemFromMercurialVaultDepositoryTest = async function (
  ...
  try {
    ...
    const collateralDelta = Number(
      (userCollateralBalance_post - userCollateralBalance_pre).toFixed(depository.collateralMint.decimals)
    );
    ...
    const estimatedCollateralAmount = Number(
      (redeemableAmount - estimatedFeesPaid).toFixed(depository.collateralMint.decimals)
    );
