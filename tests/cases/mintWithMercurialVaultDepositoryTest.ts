...
export const mintWithMercurialVaultDepositoryTest = async function (
  ...
  try {
    ...
    const collateralDelta = Number(
      (userCollateralBalance_pre - userCollateralBalance_post).toFixed(depository.collateralMint.decimals)
    );
      ...
      Math.floor(
        Math.ceil(
          nativeCollateralAmount.toNumber() -
            ((10_000 - onChainDepository_pre.mintingFeeInBps) * nativeCollateralAmount.toNumber()) / 10_000
        )
      ),
    ...
    console.log(
      `🧾 Minted`,
      Number(redeemableDelta.toFixed(depository.mercurialVaultLpMint.decimals)),
    ...
    const estimatedRedeemableAmount = Number(
      (collateralAmount - estimatedFeesPaid).toFixed(controller.redeemableMintDecimals)
    );
    ...
    expect(nativeToUi(onChainDepository_post.collateralAmountDeposited, depository.collateralMint.decimals)).equal(
      Number(
        (
          nativeToUi(onChainDepository_pre.collateralAmountDeposited, depository.collateralMint.decimals) +
          collateralAmount
    ...
    expect(nativeToUi(onChainDepository_post.mintingFeeTotalAccrued, depository.collateralMint.decimals)).equal(
      Number(
        (
          nativeToUi(onChainDepository_pre.mintingFeeTotalAccrued, depository.collateralMint.decimals) +
          estimatedFeesPaid
        ).toFixed(controller.redeemableMintDecimals)
      )
    );
