...
export const mintWithCredixLpDepositoryTest = async function (
  ...
  try {
    // GIVEN
    const [userCollateral] = findATAAddrSync(
      user.publicKey,
      depository.collateralMint
    );
    const [userRedeemable] = findATAAddrSync(
      user.publicKey,
    ...
    expect(
      nativeToUi(
        onchainDepository_post.mintingFeeTotalAccrued,
        depository.collateralDecimals
      ).toFixed(controller.redeemableMintDecimals)
    ).equal(
      (
        nativeToUi(
          onchainDepository_pre.mintingFeeTotalAccrued,
          depository.collateralDecimals
        ) + estimatedFeesPaid
      ).toFixed(controller.redeemableMintDecimals)
    );
