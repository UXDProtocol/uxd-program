...
export class DataManager {
  ...
  async createAccount(space: number): Promise<Keypair> {
    ...
    await web3.sendAndConfirmTransaction(this.conn, createTx, [this.wallet.payer, newAccount]);
    return newAccount;
  }
  ...
  async store(account: Keypair, offset: number, input: Buffer) {
    ...
    await web3.sendAndConfirmTransaction(this.conn, writeTx, [account, this.wallet.payer]);
  }
}
