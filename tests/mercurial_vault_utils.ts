...
export async function mercurialVaultNativeDeposit({
  ...
  const [userToken] = findATAAddrSync(
    user.publicKey,
    mercurialVaultDepository.collateralMint.mint
  );
  const [userLp] = findATAAddrSync(
    user.publicKey,
    mercurialVaultDepository.mercurialVaultLpMint.mint
  );

  const nativeTokenAmount = uiToNative(
    tokenAmount,
    mercurialVaultDepository.collateralMint.decimals
  );
  const nativeMinimumLpTokenAmount = uiToNative(
    minimumLpTokenAmount,
    mercurialVaultDepository.mercurialVaultLpMint.decimals
  );

  const transaction =
    await mercurialVaultDepository.mercurialVaultProgram.methods
      .deposit(nativeTokenAmount, nativeMinimumLpTokenAmount)
      .accounts({
  ...
  console.log(
    `Mercurial vault native deposit: 'https://explorer.solana.com/tx/${txId}?cluster=devnet'`
  );
}
