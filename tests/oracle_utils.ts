...
export const localSOLOraclePriceAccountKey = anchor.utils.publicKey.findProgramAddressSync(
  [solUsdSeed],
  oracle.programId
)[0];
//BTC
...
export const localBTCOraclePriceAccountKey = anchor.utils.publicKey.findProgramAddressSync(
  [btcUsdSeed],
  oracle.programId
)[0];
...
export const btcOraclePriceAccountKey = findAddr([btcUsdSeed], oracle.programId);
export const solOraclePriceAccountKey = findAddr([solUsdSeed], oracle.programId);
...
export async function create_localnet_oracle_mirrored(
  ...
  const priceOracleAccount = await connection.getAccountInfo(oraclePriceAccountKey);
  ...
  let i = 0;
  while (true) {
    const j = i + 512 > priceOracleAccount.data.length ? priceOracleAccount.data.length : i + 512;

    // console.log(`put [${i}..${j}]`);
    await oracle.rpc.put(new anchor.BN(i), priceOracleAccount.data.slice(i, j), {
      accounts: {
        buffer: localOraclePriceAccountKey,
      },
      options: TXN_OPTS,
