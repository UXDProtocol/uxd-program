...
export const mangoDepositoryMintRedeemSuite = (user: Signer, controller: Controller, depository: MangoDepository, slippage: number) => {

    it(`Redeem 100 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) when no mint has happened (should fail)`, async () => {
        try {
            await redeemWithMangoDepositoryTest(100, slippage, user, controller, depository, mango);
        } catch {
    ...
    it(`Mint 1 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) then redeem the outcome`, async () => {
        const mintedAmount = await mintWithMangoDepositoryTest(1, slippage, user, controller, depository, mango);
        await redeemWithMangoDepositoryTest(mintedAmount, slippage, user, controller, depository, mango);
    ...
    it(`Mint 1 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) then redeem the outcome (With a separate Payer)`, async () => {
        ...
        await redeemWithMangoDepositoryTest(mintedAmount, slippage, user, controller, depository, mango, payer);
        await printUserInfo(user.publicKey, controller, depository);
        await printDepositoryInfo(controller, depository, mango);
    });
    ...
    it(`Redeem 1_000_000 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) when not enough has been minted yet (should fail)`, async () => {
        try {
            await redeemWithMangoDepositoryTest(1_000_000, slippage, user, controller, depository, mango);
        } catch {
            expect(true, "Failing as planned");
        }
    ...
    it(`Mint 5 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) then redeem the outcome`, async () => {
        const mintedAmount = await mintWithMangoDepositoryTest(5, slippage, user, controller, depository, mango);
        await redeemWithMangoDepositoryTest(mintedAmount, slippage, user, controller, depository, mango);
        await printUserInfo(user.publicKey, controller, depository);
        await printDepositoryInfo(controller, depository, mango);
    });
    ...
    it(`Mint 10 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) then redeem the outcome`, async () => {
        const mintedAmount = await mintWithMangoDepositoryTest(10, slippage, user, controller, depository, mango);
        await redeemWithMangoDepositoryTest(mintedAmount, slippage, user, controller, depository, mango);
        await printUserInfo(user.publicKey, controller, depository);
        await printDepositoryInfo(controller, depository, mango);
    });
    ...
    it(`Redeem 1_000 UXD (${slippage} slippage) (should fail)`, async () => {
        try {
            await redeemWithMangoDepositoryTest(1_000, slippage, user, controller, depository, mango);
        } catch {
            expect(true, "Failing as planned");
        }
    ...
    it(`Mint 0 UXD (${slippage} slippage) (should fail)`, async () => {
        try {
            await mintWithMangoDepositoryTest(0, slippage, user, controller, depository, mango);
        } catch {
            expect(true, "Failing as planned");
        }
    ...
    it(`Redeem 0 UXD (${slippage} slippage) (should fail)`, async () => {
        try {
            await redeemWithMangoDepositoryTest(0, slippage, user, controller, depository, mango);
        } catch {
            expect(true, "Failing as planned");
        }
        expect(false, "Should have failed - Amount is 0");
    ...
    it(`Mint 1 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) 10 times then redeem the outcome`, async () => {
        ...
        for (var _i = 0; _i < 10; _i++) {
            mintedAmount += await mintWithMangoDepositoryTest(1, slippage, user, controller, depository, mango);
        }
        await redeemWithMangoDepositoryTest(mintedAmount, slippage, user, controller, depository, mango);
        await printUserInfo(user.publicKey, controller, depository);
        await printDepositoryInfo(controller, depository, mango);
    ...
    it(`Mint 10 ${depository.collateralMintSymbol} worth of UXD (${slippage} slippage) then redeem the outcome in 3 times`, async () => {
        ...
        await redeemWithMangoDepositoryTest(redeemAmountPartial, slippage, user, controller, depository, mango);
        await redeemWithMangoDepositoryTest(redeemAmountPartial, slippage, user, controller, depository, mango);
        ...
        await redeemWithMangoDepositoryTest(remainingRedeemableAmount, slippage, user, controller, depository, mango);
        await printUserInfo(user.publicKey, controller, depository);
        await printDepositoryInfo(controller, depository, mango);
    });
