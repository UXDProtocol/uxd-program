...
async fn test_identity_depository() -> Result<(), String> {
    ...
    assert_eq!(
        1_000_000,
        program_spl::accounts::get_token_account(&mut program_test_context, &user_collateral)
            .await?
            .amount
    );
    ...
    assert_eq!(
        0,
        program_spl::accounts::get_token_account(&mut program_test_context, &user_redeemable)
            .await?
            .amount
    );
