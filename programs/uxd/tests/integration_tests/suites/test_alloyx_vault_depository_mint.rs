...
#[tokio::test]
async fn test_alloyx_vault_depository_mint() -> Result<(), program_context::ProgramError> {
    // ---------------------------------------------------------------------
    // -- Phase 1
    ...
    assert!(
        program_uxd::instructions::process_mint_with_alloyx_vault_depository(
            &mut program_context,
            &payer,
            &authority,
            &collateral_mint.pubkey(),
    ...
    program_uxd::instructions::process_edit_alloyx_vault_depository(
        ...
        &EditAlloyxVaultDepositoryFields {
            redeemable_amount_under_management_cap: Some(amount_we_use_as_supply_cap),
