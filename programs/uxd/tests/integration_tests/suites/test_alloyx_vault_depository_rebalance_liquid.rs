...
async fn test_alloyx_vault_depository_rebalance_liquid() -> Result<(), program_context::ProgramError>
{
    ...
    // Airdrop collateral to our authority, collateral will be used for profit
    program_spl::instructions::process_token_mint_to(
        &mut program_context,
        &payer,
        &collateral_mint.pubkey(),
    ...
    let alloyx_vault_mint_before = program_context::read_account_packed::<Mint>(
        &mut program_context,
        &alloyx_vault_mint.pubkey(),
    )
    .await?;
    let expected_alloyx_shares_at_profit_time = amount_the_user_should_be_able_to_mint * 5 / 100;
    let expected_profits_collateral_amount = u128::from(expected_alloyx_shares_at_profit_time)
    ...
    program_uxd::instructions::process_rebalance_alloyx_vault_depository(
        ...
        &profits_beneficiary_collateral,
        i128::from(amount_the_user_should_be_able_to_mint * 5 / 100), // 5% deposit
        u64::try_from(expected_profits_collateral_amount).unwrap(),
    )
    .await?;
