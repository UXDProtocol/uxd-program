...
pub async fn read_account_data(
    ...
    let raw_account = program_runner.get_account(address).await?.ok_or(
        program_test_context::ProgramTestError::Custom("AccountDoesNotExist"),
    )?;
    Ok(raw_account.data)
}
...
pub async fn read_account_anchor<T: AccountDeserialize>(
    ...
    let raw_account_data = program_test_context::read_account_data(program_runner, address).await?;
