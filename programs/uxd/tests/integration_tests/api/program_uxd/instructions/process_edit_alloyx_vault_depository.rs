...
pub async fn process_edit_alloyx_vault_depository(
    ...
    // Execute IX
    let accounts = uxd::accounts::EditAlloyxVaultDepository {
        authority: authority.pubkey(),
        controller,
        depository: alloyx_vault_depository,
    };
    let payload = uxd::instruction::EditAlloyxVaultDepository { fields: *fields };
    ...
    let redeemable_amount_under_management_cap_before =
        alloyx_vault_depository_before.redeemable_amount_under_management_cap;
    let redeemable_amount_under_management_cap_after =
        alloyx_vault_depository_after.redeemable_amount_under_management_cap;
    assert_eq!(
    ...
    let minting_fee_in_bps_before = alloyx_vault_depository_before.minting_fee_in_bps;
    let minting_fee_in_bps_after = alloyx_vault_depository_after.minting_fee_in_bps;
    assert_eq!(
    ...
    let redeeming_fee_in_bps_before = alloyx_vault_depository_before.redeeming_fee_in_bps;
    let redeeming_fee_in_bps_after = alloyx_vault_depository_after.redeeming_fee_in_bps;
    assert_eq!(
    ...
    let minting_disabled_before = alloyx_vault_depository_before.minting_disabled;
    let minting_disabled_after = alloyx_vault_depository_after.minting_disabled;
    assert_eq!(
    ...
    let profits_beneficiary_collateral_before =
        alloyx_vault_depository_before.profits_beneficiary_collateral;
    let profits_beneficiary_collateral_after =
        alloyx_vault_depository_after.profits_beneficiary_collateral;
    assert_eq!(
