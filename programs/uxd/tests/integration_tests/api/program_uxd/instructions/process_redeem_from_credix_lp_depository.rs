...
pub async fn process_redeem_from_credix_lp_depository(
    ...
    // Execute IX
    let accounts = uxd::accounts::RedeemFromCredixLpDepository {
    ...
    let payload = uxd::instruction::RedeemFromCredixLpDepository { redeemable_amount };
    ...
    let redeeming_fee_total_accrued_before =
        credix_lp_depository_before.redeeming_fee_total_accrued;
    let redeeming_fee_total_accrued_after = credix_lp_depository_after.redeeming_fee_total_accrued;
    assert_eq!(
        redeeming_fee_total_accrued_before + u128::from(fees_amount),
        redeeming_fee_total_accrued_after,
    );

    // depository.collateral_amount_deposited must have decreaed by the withdran amount (equivalent to collateral_amount)
