...
pub fn find_program_state() -> Pubkey {
    credix_client::ProgramState::generate_pda().0
}

pub fn find_global_market_state(market_seeds: &String) -> Pubkey {
    credix_client::GlobalMarketState::generate_pda(market_seeds).0
}

pub fn find_market_admins(global_market_state: &Pubkey) -> Pubkey {
    credix_client::MarketAdmins::generate_pda(*global_market_state).0
}
...
pub fn find_credix_pass(global_market_state: &Pubkey, pass_holder: &Pubkey) -> Pubkey {
    credix_client::CredixPass::generate_pda(*global_market_state, *pass_holder).0
}
