...
pub(crate) fn handler(
    ...
    {
        msg!(
            "[transfer] {} collateral from user to his lsd_position",
            collateral_amount,
        );
        token::transfer(
            ctx.accounts
    ...
    let liquidation_price: u64;
    {
        ...
        let lsd_price = OraclePrice::new_from_oracle(
            &ctx.accounts.collateral_oracle_account.to_account_info(),
...
impl<'info> BorrowFromLsdDepository<'info> {
    pub(crate) fn validate(&self, collateral_amount: u64, loan_to_value_bps: u16) -> Result<()> {
        ...
        validate_loan_to_value_bps(
            loan_to_value_bps,
            self.depository.load()?.max_loan_to_value_bps,
        )?;
        Ok(())
    }
}
