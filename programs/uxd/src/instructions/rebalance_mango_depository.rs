...
use crate::check_short_perp_close_order_fully_filled;
use crate::check_short_perp_close_order_is_within_slippage_range;
...
pub struct RebalanceMangoDepository<'info> {
    ...
    // HERE NEED TO Pass a fee pathrough account to pay for the fees not to destabilize the system
    // #[account(
    //     mut,
    ...
    pub quote_root_bank: AccountInfo<'info>,   // read // these must be the one above
...
//   if it's positive it increase the overal delta neutral pos size
...
pub fn handler(
    ...
    // - [Perp account state PRE rebalancing]
    ...
    // todo!();
    // re load perp_acount after else wont show update

    // - [Verify PerpAccount funding is settled]
    if !(perp_account.long_settled_funding == I80F48::ZERO
        && perp_account.short_settled_funding == I80F48::ZERO)
    {
        msg!("wrong funding");
        return Err(ErrorCode::InvalidPerpAccountState.into());
    }
    ...
    let new_quote_position = I80F48::from_num(-perp_account.base_position)
        .checked_mul(contract_size)
        .unwrap()
        .checked_mul(perp_info.price)
        .unwrap();

    let pnl = perp_account
        .quote_position
