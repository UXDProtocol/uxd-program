...
use crate::ALLOYX_VAULT_EXTERNAL_PASS_NAMESPACE;
...
pub(crate) fn handler(
    ...
    let alloyx_vault_base_collateral_amount_before: u64 = ctx.accounts.alloyx_vault_collateral.amount;
    let alloyx_vault_desk_collateral_amount_before: u64 = ctx
        .accounts
        .alloyx_vault
        .wallet_desk_usdc_value;

    let total_shares_supply_before: u64 = ctx.accounts.alloyx_vault_mint.supply;
    let total_shares_value_before: u64 = checked_add(alloyx_vault_base_collateral_amount_before, alloyx_vault_desk_collateral_amount_before)?;
    ...
    credix_client::cpi::deposit_controller(
        ctx.accounts
    ...
    let alloyx_vault_base_collateral_amount_after: u64 = ctx.accounts.alloyx_vault_collateral.amount;
    let alloyx_vault_desk_collateral_amount_after: u64 = ctx
        .accounts
        .alloyx_vault
        .wallet_desk_usdc_value;

    let total_shares_supply_after: u64 = ctx.accounts.alloyx_vault_mint.supply;
    let total_shares_value_after: u64 = checked_add(alloyx_vault_base_collateral_amount_after, alloyx_vault_desk_collateral_amount_after)?;
