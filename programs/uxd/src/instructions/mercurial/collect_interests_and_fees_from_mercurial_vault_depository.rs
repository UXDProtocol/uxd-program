...
#[derive(Accounts)]
pub struct CollectInterestsAndFeesFromMercurialVaultDepository<'info> {
    /// #1
    pub interests_and_fees_redeem_authority: Signer<'info>,
    ...
    #[account(
        ...
        has_one = interests_and_fees_redeem_authority @UxdError::InvalidMercurialVaultInterestsAndFeesAuthority,
    ...
    #[account(
        mut,
        constraint = interests_and_fees_redeem_authority_collateral.mint == depository.load()?.collateral_mint @UxdError::InvalidCollateralMint,
        constraint = &interests_and_fees_redeem_authority_collateral.owner == interests_and_fees_redeem_authority.key @UxdError::InvalidOwner,
    )]
    pub interests_and_fees_redeem_authority_collateral: Box<Account<'info, TokenAccount>>,
...
pub fn handler(ctx: Context<CollectInterestsAndFeesFromMercurialVaultDepository>) -> Result<()> {
    ...
    let before_collateral_balance = ctx
        .accounts
        .interests_and_fees_redeem_authority_collateral
        .amount;

    // 1 - calculate the value of collectable interests and fees (in USDC unit)
    let collectable_interests_and_fees_value = ctx
        .accounts
        .calculate_collectable_interests_and_fees_value()?;
    ...
    let lp_token_amount_to_match_collectable_interests_and_fees_value = ctx
        .accounts
        .calculate_lp_token_amount_to_match_collectable_interests_and_fees_value(
            collectable_interests_and_fees_value,
        )?;
    ...
    mercurial_vault::cpi::withdraw(
        ...
        lp_token_amount_to_match_collectable_interests_and_fees_value,
    ...
    let after_collateral_balance = ctx
        .accounts
        .interests_and_fees_redeem_authority_collateral
        .amount;
    ...
    require!(
        lp_token_change == lp_token_amount_to_match_collectable_interests_and_fees_value,
    ...
    CollectInterestsAndFeesFromMercurialVaultDepository::check_redeemed_collateral_amount_to_match_target(
...
// Into functions
impl<'info> CollectInterestsAndFeesFromMercurialVaultDepository<'info> {
...
impl<'info> CollectInterestsAndFeesFromMercurialVaultDepository<'info> {
    fn calculate_lp_token_amount_to_match_collectable_interests_and_fees_value(
        &self,
        target_value: u64,
    ) -> Result<u64> {
    ...
    pub fn calculate_collectable_interests_and_fees_value(&self) -> Result<u64> {
        let owned_lp_tokens_value = I80F48::checked_from_num(
            self.calculate_lp_tokens_value(self.depository_lp_token_vault.amount)?,
        )
        .ok_or_else(|| error!(UxdError::MathError))?;

        let collectable_interests_and_fees_amount = owned_lp_tokens_value
            .checked_sub(
        ...
        collectable_interests_and_fees_amount
            .checked_to_num()
            .ok_or_else(|| error!(UxdError::MathError))
    }
