...
/// Takes x accounts
#[derive(Accounts)]
pub struct PullCollateralKillSwitch<'info> {
...
pub fn handler(
    ctx: Context<PullCollateralKillSwitch>,
    limit_price: f32,
    max_leverage: u8,
    collateral_factor: f64, // Better data type for this? Seems unnecessary to be f64
) -> Result<()> {
    let depository = ctx.accounts.depository.load()?;
    let collateral_mint = depository.collateral_mint;
    let depository_bump = depository.bump;

    // amount_to_withdraw = collateral_amount_deposited * (1 - 1/(max_leverage * collateral_factor))
    let percentage_collateral_needed: f64 = 1f64
        .checked_div(
            I80F48::checked_from_num(max_leverage)
            .ok_or_else(|| error!(UxdError::MathError))?
            .checked_mul(collateral_factor) // NEED TO FIX... gosh math in rust is painful at times
            .ok_or_else(|| error!(UxdError::MathError))?
        )
        .ok_or_else(|| error!(UxdError::MathError))?;
    let percentage_collateral_withdrawable: f64 = 1 - percentage_collateral_needed;
    let collateral_to_withdraw: u64 = depository.collateral_amount_deposited
        .checked_mul(percentage_collateral_withdrawable) // AH RUST MATH!!!
        .ok_or_else(|| error!(UxdError::MathError))?
        .try_into()
        .ok_or_else(|| error!(UxdError::MathError))?;
    ...
    let quote_exposure_delta = I80F48::from_num(collateral_to_withdraw);
    ...
    // SAME SWAP HERE AS THE OTHER KILL SWITCH. LOTS OF REPEATED CODE.
    // MAYBE STREAMLINE?
...
// Contexts
impl<'info> PullCollateralKillSwitch<'info> {
...
impl<'info> PullCollateralKillSwitch<'info> {
    pub fn validate(
        ...
        require!(collateral_factor <= 1f64, UxdError::CollateralFactorTooHigh);
