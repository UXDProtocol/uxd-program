...
use crate::events::ExtractMangoDepositoryCollateralDepositInterestsEvent;
...
#[derive(Accounts)]
pub struct ExtractMangoDepositoryCollateralDepositInterests<'info> {
...
pub fn handler(ctx: Context<ExtractMangoDepositoryCollateralDepositInterests>) -> Result<()> {
    ...
    // - 0 [DETERMINES THE AMOUNT OF COLLATERAL TO EXTRACT] -------------------
    let mango_account = MangoAccount::load_checked(mango_account_ai, mango_program_key, mango_group_key)
                .map_err(ProgramError::from)?;
    let mngo_deposits = mango_account
        .get_native_deposit(mngo_cache, mngo_index)
        .map_err(ProgramError::from)?;
    let amount_collateral = get_n

    // - 1 [WITHDRAW INSURANCE FROM MANGO THEN RETURN TO AUTHORITY] -----------
...
impl<'info> ExtractMangoDepositoryCollateralDepositInterests<'info> {
